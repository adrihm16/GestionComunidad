        ...
        <div class="flex flex-col gap-4" id="vecinos-list">
            ...
            @forelse ($inmuebles as $inmueble)
                ...
                @component('components.ui.card', [
                    ...
                    'bodyClass' => 'flex flex-col px-4 py-3.5 gap-2',
                ])
                @foreach($inmueble->propietarios as $owner)
                    <div class="flex items-center justify-between">
                        <a href="{{ route('vecinos.show', $owner) }}"
                            class="font-poppins font-medium text-base text-main hover:text-primary transition-colors">
                            {{ $owner->nombre }} {{ $owner->apellidos }}
                        </a>

                        @if($owner->cargo_comunidad && $owner->cargo_comunidad !== '' && $owner->cargo_comunidad !== '-')
                            @include('components.ui.badge', ['text' => $owner->cargo_comunidad, 'variant' => 'accent'])
                        @endif
                    </div>
                @endforeach

                @if($inmueble->propietarios->count() === 0)
                    <p class="text-sm text-muted italic">Sin propietario asignado</p>
                @endif
                @endcomponent
            @empty
                {{-- Empty state --}}
