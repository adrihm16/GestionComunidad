...
@section('content')
    <div class="flex flex-col gap-5">
        ...
        {{-- Noticias List --}}
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4" id="noticias-list">
                    ...
                    <div class="flex flex-col h-full">
                        ...
                        <div class="w-full h-44 overflow-hidden bg-primary/5">
                            @if($noticia->adjunto_url)
                                <img src="{{ $noticia->adjunto_url }}" alt="{{ $noticia->titulo }}"
                                    class="w-full h-full object-cover transition-transform duration-300 hover:scale-105">
                            @else
                                <div class="w-full h-full flex items-center justify-center">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-10 h-10 text-primary/20" fill="none"
                        ...
                        <div class="p-4 flex flex-col flex-1">
                            {{-- Title --}}
                            <h3 class="font-poppins font-semibold text-base text-main leading-snug mb-1.5">
                                {{ $noticia->titulo }}
                            </h3>
                            ...
                            <div class="flex items-center gap-2 text-xs text-muted mt-auto pt-2">
                                ...
                                <span>{{ $noticia->autor->nombre }} {{ $noticia->autor->apellidos }}</span>
                            </div>
                        </div>
                    </div>
                    @endcomponent
