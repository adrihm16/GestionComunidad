...
@section('content')
    <div class="flex flex-col gap-5">
        ...
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('admin.users.create') }}" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-primary text-white text-sm font-medium shadow-md
        ...
        <form method="GET" action="{{ route('admin.users.index') }}" class="flex flex-col md:flex-row gap-3">
            ...
            <div class="flex-1">
                <input type="text" name="search" value="{{ request('search') }}"
                    placeholder="Buscar por nombre, apellidos o email..." class="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm text-main
            ...
            @if(request('search') || request('rol'))
                <a href="{{ route('admin.users.index') }}" class="px-4 py-2.5 rounded-xl bg-gray-100 text-main text-sm font-medium
                    ...
                    <div class="flex items-center flex-wrap gap-2">
                        ...
                        @if($user->inmuebles->count() > 0)
                            <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg text-xs font-semibold bg-emerald-100 text-emerald-700">
                                ...
                                {{ $user->inmuebles->count() }} {{ $user->inmuebles->count() === 1 ? 'Inmueble' : 'Inmuebles' }}
                            </span>
                        @else
                            <span class="inline-flex items-center px-3 py-1 rounded-lg text-xs font-medium bg-gray-100 text-gray-400 italic">
                                Sin inmuebles
                            </span>
                        @endif
                        ...
                        @if($user->cargo_comunidad)
                            <span
                                class="hidden sm:inline-flex items-center px-3 py-1 rounded-lg text-xs font-medium bg-amber-100 text-amber-700">
