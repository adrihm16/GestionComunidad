...
@section('content')
    <div class="flex flex-col gap-5 max-w-2xl mx-auto">
        ...
        <form method="POST" action="{{ route('admin.users.update', $user) }}" class="flex flex-col gap-4">
            ...
            {{-- User Info Badge --}}
            <div
                class="flex items-center gap-3 p-3 rounded-lg bg-blue-50 dark:bg-blue-950/30 border border-blue-100 dark:border-blue-900/40">
                ...
                <div>
                    <p class="text-sm font-semibold text-blue-900 dark:text-blue-100">{{ $user->nombre }}
                        {{ $user->apellidos }}
                    </p>
                    <p class="text-xs text-blue-700 dark:text-blue-300">{{ $user->email }}</p>
                </div>
            </div>
            ...
            <div>
                ...
                <input type="text" name="nombre" value="{{ old('nombre', $user->nombre) }}" required class="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-sm text-main dark:text-gray-100
            ...
            <div>
                <label class="block text-sm font-medium text-main dark:text-gray-300 mb-1.5">Apellidos <span
                        class="text-red-500">*</span></label>
                <input type="text" name="apellidos" value="{{ old('apellidos', $user->apellidos) }}" required class="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-sm text-main dark:text-gray-100
                                      focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary
            ...
            <div>
                <label class="block text-sm font-medium text-main dark:text-gray-300 mb-1.5">Email <span
                        class="text-red-500">*</span></label>
                <input type="email" name="email" value="{{ old('email', $user->email) }}" required class="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-sm text-main dark:text-gray-100
                                      focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary
            ...
            <div>
                ...
                <input type="tel" name="telefono" value="{{ old('telefono', $user->telefono) }}" placeholder="666555444"
            ...
            <div class="border-t border-gray-100 dark:border-gray-700 pt-4">
                <p class="text-sm font-semibold text-main dark:text-gray-100 mb-1">Cambiar contraseña (opcional)</p>
                <p class="text-xs text-muted dark:text-gray-400">Deja estos campos vacíos si no deseas cambiar la contraseña
                </p>
            </div>

            {{-- Password --}}
            <div>
                <label class="block text-sm font-medium text-main dark:text-gray-300 mb-1.5">Nueva contraseña</label>
                <input type="password" name="password" class="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-sm text-main dark:text-gray-100
                                      focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary
            ...
            {{-- Confirm Password --}}
            <div>
                <label class="block text-sm font-medium text-main dark:text-gray-300 mb-1.5">Confirmar nueva
                    contraseña</label>
                <input type="password" name="password_confirmation" class="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-sm text-main dark:text-gray-100
                                      focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary
            ...
            <div
                ...
                <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full sm:w-auto">
                    <a href="{{ route('admin.users.index') }}" class="px-5 py-2.5 rounded-xl text-sm font-medium text-muted dark:text-gray-300 text-center
        ...
        <div>
            ...
            @if($user->inmuebles->count() === 0 && session('success'))
                <div
                    class="flex items-center gap-3 p-3 mb-4 rounded-lg bg-blue-50 dark:bg-blue-950/30 border border-blue-100 dark:border-blue-900/40">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-blue-600 dark:text-blue-400 flex-shrink-0"
                        fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round"
                            d="M11.25 11.25l.041-.02a.75.75 0 011.063.852l-.708 2.836a.75.75 0 001.063.853l.041-.021M21 12a9 9 0 11-18 0 9 9 0 0118 0zm-9-3.75h.008v.008H12V8.25z" />
                    </svg>
                    <div>
                        <p class="text-sm font-semibold text-blue-900 dark:text-blue-100">Siguiente paso: Asigna inmuebles</p>
                        <p class="text-xs text-blue-700 dark:text-blue-300">Este usuario no tiene inmuebles asignados. Usa el
                            botón de abajo para agregar uno.</p>
                    </div>
                </div>
            @endif
            ...
            @if($user->inmuebles->count() > 0)
                <div class="flex flex-col gap-3 mb-4">
                    @foreach($user->inmuebles as $inmueble)
                        <div
                            class="flex items-center justify-between p-3 rounded-lg bg-gray-50 dark:bg-emerald-900/10 border border-gray-100 dark:border-emerald-900/30">
                            <div class="flex items-center gap-3">
                                {{-- Icon based on type --}}
                                ...
                                {{-- Inmueble Details --}}
                                <div>
                                    <p class="text-sm font-semibold text-main dark:text-gray-100">
                            ...
                            <form method="POST" action="{{ route('admin.users.inmuebles.destroy', [$user, $inmueble]) }}"
                                onsubmit="return confirm('¿Eliminar este inmueble?')" class="inline">
                                @csrf
                                @method('DELETE')
            ...
            @else
                <p class="text-sm text-muted dark:text-gray-400 mb-4">Este usuario no tiene inmuebles asignados.</p>
            @endif
            ...
            <div x-data="{ open: false, tipo: '' }" class="border-t border-gray-100 pt-4">
                ...
                {{-- Form --}}
                <form method="POST" action="{{ route('admin.users.inmuebles.store', $user) }}" x-show="open" x-transition
                    x-data="{ submitting: false }" @submit="submitting = true" class="mt-4 flex flex-col gap-4">
                    @csrf

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {{-- Tipo --}}
                        <div>
                            <label class="block text-sm font-medium text-main dark:text-gray-300 mb-1.5">Tipo <span
                                    class="text-red-500">*</span></label>
                            <select name="tipo" x-model="tipo" required class="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-sm text-main dark:text-gray-100
                                                   focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary
                                                   transition-all duration-200">
                        ...
                        {{-- Bloque --}}
                        <div>
                            <label class="block text-sm font-medium text-main dark:text-gray-300 mb-1.5">Bloque</label>
                            <input type="text" name="bloque" placeholder="A, B, C..." maxlength="10" class="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-sm text-main dark:text-gray-100
                                                  focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary
                                                  transition-all duration-200">
                        </div>
                        ...
                        <template x-if="tipo === 'piso' || tipo === 'local'">
                            <div class="col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4">
                                {{-- Piso --}}
                                <div>
                                    <label class="block text-sm font-medium text-main dark:text-gray-300 mb-1.5">Piso <span
                                            class="text-red-500">*</span></label>
                                ...
                                {{-- Puerta --}}
                                <div>
                                    <label class="block text-sm font-medium text-main dark:text-gray-300 mb-1.5">Puerta
                        ...
                        <template x-if="tipo === 'garaje' || tipo === 'trastero'">
                            <div class="col-span-2">
                                <label class="block text-sm font-medium text-main dark:text-gray-300 mb-1.5">
                                    Número/Identificador <span class="text-red-500">*</span>
                                </label>
                                <input type="text" name="piso" placeholder="G1, G2, T1, T2, A, B, 1, 2..." maxlength="10"
                                    required class="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-sm text-main dark:text-gray-100
                                                      focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary
                                                      transition-all duration-200">
