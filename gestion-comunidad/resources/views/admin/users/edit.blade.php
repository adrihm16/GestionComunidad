    ...
    <div class="flex flex-col gap-5 max-w-2xl mx-auto">
        ...
        <form id="user-update-form" method="POST" action="{{ route('admin.users.update', $user) }}"
            ...
            <div>
                ...
                <input type="text" name="nombre" value="{{ old('nombre', $user->nombre) }}" required @class([
            ...
            <div>
                ...
                <input type="text" name="apellidos" value="{{ old('apellidos', $user->apellidos) }}" required @class([
                    'w-full px-4 py-2.5 rounded-xl border text-sm text-main focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-all duration-200',
            ...
            <div>
                ...
                <p class="text-[11px] text-muted mt-1">Solo visible para administradores y el vecino titular.</p>
                @error('iban')
                    <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="password" name="password" class="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm text-main
                                                  focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary
            ...
            <div>
                ...
                <input type="password" name="password_confirmation" class="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm text-main
                                                  focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary
        ...
        <div
            ...
            <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full sm:w-auto">
                ...
                <button type="submit" form="user-update-form" class="inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl 
            ...
            @if($user->inmuebles->count() === 0 && session('success'))
                <div class="flex items-center gap-3 p-3 mb-4 rounded-lg bg-blue-50 border border-blue-100">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-blue-700">Este usuario no tiene inmuebles asignados. Usa el botón de abajo para
                            agregar uno.</p>
                    </div>
                </div>
            @endif
            ...
            @if($user->inmuebles->count() > 0)
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-4">
                    @foreach($user->inmuebles as $inmueble)
                        <div
                            class="flex items-center justify-between p-3 rounded-xl bg-gray-50 border border-gray-100 hover:border-primary/20 transition-colors">
                            <div class="flex items-center gap-3">
                                {{-- Icon based on type --}}
                                <div
                            ...
                            <form method="POST" action="{{ route('admin.users.inmuebles.destroy', [$user, $inmueble]) }}"
                                ...
                                @csrf
                                @method('DELETE')
                                <input type="hidden" name="redirect_to" value="#inmuebles">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <template x-if="tipo === 'piso' || tipo === 'local'">
                            <div class="col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4">
                                {{-- Piso --}}
                                <div>
                                    <label class="block text-sm font-medium text-main mb-1.5">Piso <span
                                            class="text-red-500">*</span></label>
