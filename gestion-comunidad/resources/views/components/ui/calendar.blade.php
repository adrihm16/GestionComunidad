...
<div id="calendar-widget">
    ...
    <div class="flex items-center justify-between mb-3">
        <button id="cal-prev" class="p-1.5 rounded-lg hover:bg-primary/10 transition-colors" aria-label="Mes anterior">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-main dark:text-gray-100" fill="none"
                viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 19.5L8.25 12l7.5-7.5" />
            </svg>
        </button>
        <span id="cal-month-label" class="font-semibold text-sm text-main dark:text-gray-100">
    ...
    <div class="grid grid-cols-7 gap-1 mb-1">
        @foreach(['L', 'M', 'X', 'J', 'V', 'S', 'D'] as $day)
            <div class="text-center text-xs font-medium text-muted dark:text-emerald-500/60 py-1">{{ $day }}</div>
        @endforeach
    </div>
    ...
    <div id="cal-grid" class="grid grid-cols-7 gap-1 transition-opacity duration-200">
        ...
        @for($day = 1; $day <= $calendarData['daysInMonth']; $day++)
            ...
            : 'text-main dark:text-gray-100 hover:bg-primary/10 dark:hover:bg-emerald-900/30' }}">
                {{ $day }}
                @if($hasEvent && !$isToday)
    ...
    {{-- Events legend --}}
    <div id="cal-events"
        class="mt-3 pt-3 border-t border-gray-100 dark:border-emerald-900/20 transition-opacity duration-200">
            ...
            <div class="flex flex-col gap-2">
                @foreach($calendarData['events'] as $event)
                    <div class="flex items-start gap-2 text-xs">
                        ...
                        <div class="flex-1">
                            <span class="font-medium text-main dark:text-gray-100">{{ $event['titulo'] }}</span>
                            <span class="text-muted dark:text-emerald-500/60 ml-1">
                                — {{ $event['fecha_inicio'] }}
                                @if($event['multiday'])
                                    → {{ $event['fecha_fin'] }}
                                @else
                        ...
                        @php
                            $tipoBadge = match ($event['tipo']) {
                                'junta' => 'bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-accent',
                                'mantenimiento' => 'bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400',
                                'obra' => 'bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-400',
                                default => 'bg-gray-100 dark:bg-emerald-900/20 text-gray-600 dark:text-emerald-500/60',
                            };
                        @endphp
                    ...
                    if (data.events && data.events.length > 0) {
                        ...
                        data.events.forEach(ev => {
                            const badges = {
                                ...
                                'mantenimiento': 'bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400',
                                'obra': 'bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-400',
                                'otro': 'bg-gray-100 dark:bg-emerald-900/20 text-gray-600 dark:text-emerald-500/60',
                            };
                            ...
                            <div class="flex items-start gap-2 text-xs">
                                <span class="w-1.5 h-1.5 rounded-full bg-accent mt-1.5 shrink-0"></span>
                                <div class="flex-1">
                                    <span class="font-medium text-main dark:text-gray-100">${ev.titulo}</span>
                                    <span class="text-muted dark:text-emerald-500/60 ml-1">— ${dateText}</span>
                                </div>
