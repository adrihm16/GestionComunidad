                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-2">
                                <span>{{ $recibo->inmueble->propietario->nombre }} {{ $recibo->inmueble->propietario->apellidos }}</span>
                                @if($recibo->inmueble->propietario->cargo_comunidad)
                                    <span class="px-2 py-0.5 rounded-full bg-blue-100 text-blue-700 text-[10px] font-semibold tracking-wide uppercase">
                                        {{ $recibo->inmueble->propietario->cargo_comunidad }}
                                    </span>
                                @endif
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                ...
                                {{-- Delete Button --}}
                                <form action="{{ route('admin.recibos.destroy', $recibo) }}" method="POST" onsubmit="return confirm('¿Estás seguro de que deseas eliminar este recibo?');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="p-1 text-muted hover:text-red-600 transition-colors" title="Eliminar recibo">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
